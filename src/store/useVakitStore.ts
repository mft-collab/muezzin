...
export const useVakitStore = create<VakitState>((set, get) => ({
  ...
  _processData: () => {
    ...
    const bugunChanged = JSON.stringify(prevBugun) !== JSON.stringify(newBugun);
    const yarinChanged = JSON.stringify(prevYarin) !== JSON.stringify(newYarin);

    if (bugunChanged || yarinChanged) {
      set({ bugunVakitler: newBugun, yarinVakitler: newYarin, loading: false, initialized: true });
